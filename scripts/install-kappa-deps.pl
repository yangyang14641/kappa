...
# base dir for kappa++
my $whereami = cwd;
my $kappa="$whereami/.."; # lunch this script from where it is!
...
my $opt_with_cuda     = 0;
my $opt_static        = "0";
my $opt_install_dir   = "$kappa/kappa_deps/$arch";
...
my $opt_cuda_bin  = "";
my $opt_cmake_dir     = "";
my $opt_confile       = "kappa.conf";
my $opt_tmp_dir       = "$kappa/kappa_tmp";
my $opt_build         = "optim";
my $opt_dwnldsrc      = "";
...
#    "cmake"      => [ "3.9.2",   'on',  'off', $priority++,  \&install_cmake ],
...
#    "openmpi"    => [ "1.6.5",  'on' ,  'off', $priority++,  \&install_openmpi ],
...
sub parse_commandline() # Parse command line
{
    ...
    {
      print <<ZZZ;
install-kappa-deps.pl : Install software dependencies for COOLFluiD

usage: install-kappa-deps.pl [options]
...
options:
        ...
        --genconf            Generate a kappa.conf based on the installed dependencies
        ...
        --debug=             Compile dependencies and kappa with debug symbols (=1 is default)
...
# ./install-kappa-deps.pl --install-dir=\$opt_install_dir --install-mpi-dir=\$opt_install_mpi_dir --tmp-dir=\$opt_tmp_dir
...
./install-kappa-deps.pl --install-dir=\$opt_install_dir --install-mpi-dir=\$opt_install_mpi_dir --tmp-dir=\$opt_tmp_dir --cuda-dir=\$opt_cuda_dir

#installation of individual (MPI-dependent) packages in \$opt_install_mpi_dir from \$opt_tmp_dir
./install-kappa-deps.pl --install=mpich2,petsc,parmetis --install-mpi-dir=\$opt_install_mpi_dir --tmp-dir=\$opt_tmp_dir

# installation of petsc in optimized mode and using long long int
./install-kappa-deps.pl --install=petsc --int64=1 --install-petsc-dir=\$opt_install_mpi_dir/petsc_optim_long --debug=0 --install-mpi-dir=\$opt_install_mpi_dir
...
sub install_yamlcpp() {
    ...
    run_command_or_die("unzip yaml-cpp.zip ; mv yaml-cpp-master yaml-cpp");
    safe_chdir("$opt_tmp_dir/yaml-cpp");

    run_command_or_die("cmake -D CMAKE_INSTALL_PREFIX:PATH=$opt_install_dir -D BUILD_SHARED_LIBS:BOOL=ON .");
...
#==========================================================================
 ...
 sub install_armadillo() {
  ...
  unless ($opt_fetchonly) {
   rmtree "$opt_tmp_dir/$lib-$version";
   run_command_or_die("tar -xvf armadillo-8.100.1.tar.xz");
   safe_chdir("$opt_tmp_dir/$lib-$version");

  run_command_or_die("cmake -D CMAKE_INSTALL_PREFIX:PATH=$opt_install_dir    \\
  ...
  run_command_or_die("make $opt_makeopts");
  run_command_or_die("make install");
  run_command_or_die("ln -s $opt_install_dir/lib64/libarmadillo.so $opt_install_dir/lib/libarmadillo.so");
 }
}
...
#   rmtree "$opt_tmp_dir/$lib-$version";
    untar_src($lib,$version);
    run_command_or_die("mv BLAS-3.7.1 $lib-$version");
    
# if untar, rename is needed
...
#      safe_copy("libblas.a","$opt_install_dir/lib/libblas.a");
      run_command_or_die("make all $opt_makeopts");
      run_command_or_die("gfortran -shared -fPIC -O3 -o libblas.so.$version *.f");
      safe_copy("libblas.so.$version","$opt_install_dir/lib/libblas.so.$version") or die;
      run_command_or_die("cp blas_LINUX.a libblas.a");
      safe_copy("blas_LINUX.a","$opt_install_dir/lib/libblas.a");
...
#      safe_copy("liblapack.a","$opt_install_dir/lib/liblapack.a");
      ...
      run_command_or_die("cp $opt_tmp_dir/$lib-$version/INSTALL/make.inc.gfortran $opt_tmp_dir/$lib-$version/make.inc");

      run_command_or_die("make all $opt_makeopts");

      run_command_or_die("gfortran -shared -fPIC -O3 -o liblapack.so.$version *.f");
      safe_copy("liblapack.so.$version","$opt_install_dir/lib/liblapack.so.$version") or die;
      safe_copy("$opt_tmp_dir/$lib-$version/liblapack.a","$opt_install_dir/lib/liblapack.a");
...
#   if ( not -e "$pack.tar.bz2" ) {
    ...
    run_command_or_die("wget http://vorboss.dl.sourceforge.net/project/boost/boost/$mversion/$pack.tar.gz");
#  }
  ...
  unless ($opt_fetchonly)
  {
    ...
    run_command_or_die("tar xvf $lib\_$version.tar.gz");
    safe_chdir("$opt_tmp_dir/$pack/");
